"""Comp-off routes — overtime compensation management."""
...
def _calculate_ot_hours(db: Session, user_id: int, start_date: datetime, end_date: datetime) -> float:
    ...
    total_ot = sum(
        max(0, (r.hours_worked or 0) - standard_hours)
        for r in records
    )
...
def request_compoff(
    data: CompOffCreate,
    current_user: User = Depends(require(Permission.CREATE_OWN_COMPOFF)),
    db: Session = Depends(get_db)
):
    """Request comp-off conversion for overtime hours."""
    ot_hours = _calculate_ot_hours(db, current_user.id, data.ot_start_date, data.ot_end_date)
    if ot_hours < 4:
    ...
    compoff = CompOff(
        user_id=current_user.id, ot_hours=ot_hours, comp_off_days=comp_off_days,
        ot_start_date=data.ot_start_date, ot_end_date=data.ot_end_date, reason=data.reason
    )
    db.add(compoff)
    db.commit()
    db.refresh(compoff)
...
def get_my_compoff_requests(
    ...
    current_user: User = Depends(require(Permission.VIEW_OWN_COMPOFF)),
    db: Session = Depends(get_db)
):
...
def get_all_compoff_requests(
    ...
    current_user: User = Depends(require(Permission.MANAGE_COMPOFF)),
    db: Session = Depends(get_db)
):
...
def get_compoff_balance(
    ...
    approved = db.query(func.sum(CompOff.comp_off_days)).filter(
        CompOff.user_id == current_user.id, CompOff.status == CompOffStatus.APPROVED
    ).scalar() or 0
    used = db.query(func.sum(CompOff.comp_off_days)).filter(
        CompOff.user_id == current_user.id, CompOff.status == CompOffStatus.USED
    ).scalar() or 0
    return {"approved_days": float(approved), "used_days": float(used), "available_days": float(approved) - float(used)}
